    ...
    if (arg.startsWith('--')) {
      ...
      if (rawName.startsWith('no-')) {
        ...
        if (inlineValue !== undefined) {
          throw new Error(`Boolean flag --${rawName} does not accept a value`);
        }
        options[definition.name] = false;
        provided.add(definition.name);
        continue;
      }
      ...
      if (definition.type === 'boolean') {
        if (inlineValue !== undefined) {
          throw new Error(`Boolean flag --${rawName} does not accept a value, use --${rawName} or --no-${rawName}`);
        }
