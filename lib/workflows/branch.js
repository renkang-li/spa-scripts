...
function buildBranchPlan(project, branchName, rawOptions = {}) {
  ...
  const localExists = localBranchExists(project.path, branchName);
  const remoteExists = remoteBranchExists(project.path, branchName);
  const baseResult = (!localExists && !remoteExists)
    ? resolveBaseRef(project.path, options.base)
...
function createBranchBatchPlan(projects, branchName, rawOptions = {}) {
  const options = normalizeOptions(rawOptions);
  const plans = projects.map((project) => buildBranchPlan(project, branchName, options));
...
function executeSinglePlan(plan, options) {
  const result = {
    project: plan.project.name,
  ...
  if (plan.action === 'checkout-local') {
    const checkoutResult = checkoutLocalBranch(plan.project.path, plan.branchName);
  ...
  if (plan.action === 'checkout-remote') {
    const checkoutResult = checkoutRemoteBranch(plan.project.path, plan.branchName);
    if (!checkoutResult.success) {
...
function executeBranchBatch(batchPlan, rawOptions = {}) {
  const options = normalizeOptions(rawOptions);
  ...
  return {
    ...batchPlan,
    executed: true,
    results: batchPlan.plans.map((plan) => executeSinglePlan(plan, options)),
  };
}
