...
function buildVersionPreview(project, options) {
  const isRemote = options.sourceMode === 'remote-branch' && options.remoteBranch;
  const originFetchResult = isRemote
  ...
  if (isRemote && !remoteBranchExists(project.path, options.remoteBranch)) {
    return {
      project,
      fetchError: fetchResult.success ? null : (fetchResult.combined || fetchResult.error),
      sourceRef,
...
function createVersionBatchPlan(projects, rawOptions = {}) {
  ...
  return {
    options,
    previews: projects.map((project) => buildVersionPreview(project, options)),
  };
}
...
function executeVersionPreview(preview, options) {
  ...
  const createResult = isRemote
    ? createAnnotatedTagAt(preview.project.path, preview.version, message, preview.sourceRef)
    : createAnnotatedTag(preview.project.path, preview.version, message);
