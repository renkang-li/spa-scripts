...
function buildProjectRows(projects) {
  return mapConcurrent(projects, DEFAULT_CONCURRENCY, async (project) => ({
    name: project.name,
    currentBranch: (await getCurrentBranch(project.path)) || 'unknown',
  }));
}
...
async function run(argv) {
  ...
  try {
    const projectRows = await buildProjectRows(allProjects);
    ...
    const batchPlan = await createBranchBatchPlan(selectedResult.selected, validation.branchName, options);
    ...
    const execution = await executeBranchBatch(batchPlan, options);
